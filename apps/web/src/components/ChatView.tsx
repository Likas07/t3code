...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const activeDelegationBatch = useMemo(() => {
    if (!activeThread || activeThread.lineage.role !== "primary") {
      return null;
    }
    return (
      activeThread.delegationBatches.find((batch) => batch.status === "running") ??
      activeThread.delegationBatches.at(-1) ??
      null
    );
  }, [activeThread]);
  const managedParentThread = useMemo(() => {
    if (
      !activeThread ||
      activeThread.lineage.role !== "child" ||
      activeThread.lineage.parentThreadId === null
    ) {
      return null;
    }
    return threadById.get(activeThread.lineage.parentThreadId) ?? null;
  }, [activeThread, threadById]);
  const managedParentBatch = useMemo(() => {
    if (!activeThread || activeThread.lineage.role !== "child" || !managedParentThread) {
      return null;
    }
    if (!activeThread.lineage.parentBatchId) {
      return null;
    }
    return (
      managedParentThread.delegationBatches.find(
        (batch) => batch.batchId === activeThread.lineage.parentBatchId,
      ) ?? null
    );
  }, [activeThread, managedParentThread]);
  const isManagedChildThread = managedParentBatch?.status === "running";
  const activeBlockedDelegationChildren = useMemo(() => {
    if (!activeDelegationBatch || activeDelegationBatch.status !== "running") {
      return [];
    }
    return activeDelegationBatch.children.flatMap((child) => {
      if (child.status !== "blocked") {
        return [];
      }
      const childThread = threadById.get(child.childThreadId);
      if (!childThread) {
        return [];
      }
      return [
        {
          child,
          thread: childThread,
          approvals: derivePendingApprovals(childThread.activities),
          userInputs: derivePendingUserInputs(childThread.activities),
        },
      ];
    });
  }, [activeDelegationBatch, threadById]);
  const delegatedApprovalSource = useMemo(
    () => activeBlockedDelegationChildren.find((entry) => entry.approvals.length > 0) ?? null,
    [activeBlockedDelegationChildren],
  );
  const delegatedUserInputSource = useMemo(
    () => activeBlockedDelegationChildren.find((entry) => entry.userInputs.length > 0) ?? null,
    [activeBlockedDelegationChildren],
  );
  ...
  const threadPendingUserInputs = useMemo(
    () => derivePendingUserInputs(threadActivities),
    [threadActivities],
  );
  const pendingApprovals =
    threadPendingApprovals.length > 0
      ? threadPendingApprovals
      : (delegatedApprovalSource?.approvals ?? EMPTY_PENDING_APPROVALS);
  const pendingApprovalTargetThreadId =
    threadPendingApprovals.length > 0
      ? activeThreadId
      : (delegatedApprovalSource?.thread.id ?? null);
  const pendingApprovalSourceThreadTitle =
    threadPendingApprovals.length > 0 ? null : (delegatedApprovalSource?.thread.title ?? null);
  const pendingUserInputs =
    threadPendingUserInputs.length > 0
      ? threadPendingUserInputs
      : (delegatedUserInputSource?.userInputs ?? EMPTY_PENDING_USER_INPUTS);
  const pendingUserInputTargetThreadId =
    threadPendingUserInputs.length > 0
      ? activeThreadId
      : (delegatedUserInputSource?.thread.id ?? null);
  const pendingUserInputSourceThreadTitle =
    threadPendingUserInputs.length > 0 ? null : (delegatedUserInputSource?.thread.title ?? null);
  ...
  const isComposerLockedByDelegation =
    isManagedChildThread &&
    threadPendingApprovals.length === 0 &&
    threadPendingUserInputs.length === 0;
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    if (isComposerLockedByDelegation) {
      return;
    }
  ...
  const onRespondToApproval = useCallback(
    async (requestId: ApprovalRequestId, decision: ProviderApprovalDecision) => {
      const api = readNativeApi();
      const targetThreadId = pendingApprovalTargetThreadId ?? activeThreadId;
      if (!api || !targetThreadId) return;
    ...
    [activeThreadId, pendingApprovalTargetThreadId, setStoreThreadError],
  );

  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      const api = readNativeApi();
      const targetThreadId = pendingUserInputTargetThreadId ?? activeThreadId;
      if (!api || !targetThreadId) return;
    ...
    [activeThreadId, pendingUserInputTargetThreadId, setStoreThreadError],
  );
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      />
      {isManagedChildThread && managedParentThread ? (
        <ManagedChildThreadBanner
          parentThreadTitle={managedParentThread.title}
          onOpenParent={() => {
            void navigate({
              to: "/$threadId",
              params: { threadId: managedParentThread.id },
            });
          }}
        />
      ) : null}
      ...
      {activeDelegationBatch ? (
        <DelegationPanel
          batch={activeDelegationBatch}
          childThreadsById={threadById}
            ...
            {activePendingApproval ? (
              <div className="rounded-t-[19px] border-b border-border/65 bg-muted/20">
                <ComposerPendingApprovalPanel
                  ...
                  sourceThreadTitle={pendingApprovalSourceThreadTitle}
                />
              </div>
            ) : pendingUserInputs.length > 0 ? (
              <div className="rounded-t-[19px] border-b border-border/65 bg-muted/20">
                <ComposerPendingUserInputPanel
                  ...
                  sourceThreadTitle={pendingUserInputSourceThreadTitle}
              ...
              <ComposerPromptEditor
                ...
                placeholder={
                  isComposerApprovalState
                    ...
                    : isComposerLockedByDelegation
                      ? "Managed by parent thread until the delegation batch completes"
                    : showPlanFollowUpPrompt && activeProposedPlan
                ...
                disabled={isConnecting || isComposerApprovalState || isComposerLockedByDelegation}
              />
            </div>
                    ...
                    showPlanFollowUpPrompt ? (
                      prompt.trim().length > 0 ? (
                        <Button
                          ...
                          disabled={isSendBusy || isConnecting || isComposerLockedByDelegation}
                        >
                            ...
                            <MenuTrigger
                              render={
                                <Button
                                  ...
                                  disabled={
                                    isSendBusy || isConnecting || isComposerLockedByDelegation
                                  }
                                />
                              }
                            >
...
const ManagedChildThreadBanner = memo(function ManagedChildThreadBanner({
  parentThreadTitle,
  onOpenParent,
}: {
  ...
  return (
    <div className="pt-3 mx-auto w-full max-w-3xl px-3 sm:px-5">
      <Alert variant="warning">
        <CircleAlertIcon />
        <AlertTitle>Managed by parent thread</AlertTitle>
        <AlertDescription>
          This delegated child thread is locked until its parent batch completes.
        </AlertDescription>
        <AlertAction>
          <Button size="sm" variant="outline" onClick={onOpenParent}>
            Open {truncateTitle(parentThreadTitle, 28)}
          </Button>
        </AlertAction>
      </Alert>
    </div>
...
function delegationStatusBadgeClass(status: Thread["delegationBatches"][number]["children"][number]["status"]): string {
  switch (status) {
    case "completed":
      return "bg-emerald-500/10 text-emerald-600 dark:text-emerald-300/90";
    case "failed":
      return "bg-rose-500/10 text-rose-600 dark:text-rose-300/90";
    case "blocked":
      return "bg-amber-500/10 text-amber-600 dark:text-amber-300/90";
    case "running":
      return "bg-sky-500/10 text-sky-600 dark:text-sky-300/90";
    case "queued":
      return "bg-muted text-muted-foreground";
    default:
      return "bg-zinc-500/10 text-zinc-500 dark:text-zinc-300/80";
  }
}

const DelegationPanel = memo(function DelegationPanel({
  batch,
  childThreadsById,
  onOpenThread,
}: {
  batch: Thread["delegationBatches"][number];
  childThreadsById: ReadonlyMap<ThreadId, Thread>;
      ...
      <div className="rounded-2xl border border-border/70 bg-card/70 p-3 sm:p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] text-foreground/80">
            {batch.workspaceMode === "same-worktree" ? "Same worktree" : "Separate worktrees"}
          </span>
          <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] text-foreground/80">
        ...
        <div className="mt-3 space-y-2">
          {batch.children.map((child) => {
            const childThread = childThreadsById.get(child.childThreadId);
            const detail = child.summary ?? child.error;
            const location = child.worktreePath ?? child.branch;
            return (
              <button
                key={child.childThreadId}
                type="button"
                onClick={() => onOpenThread(child.childThreadId)}
                className="flex w-full flex-col gap-2 rounded-xl border border-border/60 bg-background/60 px-3 py-2 text-left transition-colors hover:bg-muted/30"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-medium text-foreground/90">{child.title}</span>
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] ${delegationStatusBadgeClass(child.status)}`}
                  >
                    {child.status}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    Task {child.taskIndex + 1}
                  </span>
                  {child.blockingKind ? (
                    <span className="text-xs text-muted-foreground">
                      Waiting on {child.blockingKind === "user-input" ? "user input" : "approval"}
                    </span>
                  ) : null}
                </div>
                {location ? (
                  <div className="text-xs text-muted-foreground/70" title={location}>
                    {location}
                  </div>
                ) : null}
                {detail ? (
                ...
                {childThread?.title && childThread.title !== child.title ? (
                  <div className="text-[11px] text-muted-foreground/60">
                    Thread: {childThread.title}
                  </div>
                ) : null}
              </button>
            );
...
interface ComposerPendingApprovalPanelProps {
  ...
  pendingCount: number;
  sourceThreadTitle?: string | null | undefined;
}
  ...
  return (
    <div className="px-4 py-3.5 sm:px-5 sm:py-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {sourceThreadTitle ? (
          <span className="text-xs text-muted-foreground">Child: {sourceThreadTitle}</span>
        ) : null}
      </div>
    </div>
  );
    ...
    <div className="px-4 py-3 sm:px-5">
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          {sourceThreadTitle ? (
            <span className="text-[10px] text-muted-foreground/55">Child: {sourceThreadTitle}</span>
          ) : null}
        </div>
      </div>
